...
public:
  ...
  void run() {
    ...
    std::cout << "8. Generar grafo aleatorio\n";
    std::cout << "9. Generar y visualizar grafo\n";
    std::cout << "10. Encontrar camino más corto con Dijkstra\n";
    std::cout << "0. Salir\n";
    ...
    do {
      ...
      case 8: {
        ...
        bool isDirected;
        std::vector<std::string> labels;
        int numNodes, maxEdgePerNode, maxWeight;

        std::cout << "El grafo sera directo(responder con SI o NO): ";
        std::cin >> resUser;
        std::cout << "El numero de nodos sera, numero aristas por nodo, peso "
                     "maximo: ";
        std::cin >> numNodes >> maxEdgePerNode >> maxWeight;

        isDirected = resUser == "si" ? true : false;

        for (int i = 0; i <= numNodes; i++) {
          std::string labelChar;
          std::cout << "Indique el label del nodo " << i << ": ";
          std::cin >> labelChar;
          labels.push_back(labelChar);
        }
        delete _currentGraph;
        _currentGraph = new Graph(numNodes, labels, isDirected);
        _currentGraph->buildRamdonGraph(maxEdgePerNode, maxWeight);
        _currentGraph->showGraph();
